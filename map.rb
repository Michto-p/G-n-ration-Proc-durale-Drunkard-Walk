...
class Map
  ...
  def generate_walker_map(size_x=50,size_y=50)
		...
		walker[0] = rand(1..size_x-2),rand(1..size_y-2),rand(0..3) # génération du premier walker*
  ...
  def button_down(id)
	if id ==  KB_SPACE
		@map = generate_walker_map(100,100) 
		@t_stack = -1
	end
	puts "NEW MAP!!!"
  end
